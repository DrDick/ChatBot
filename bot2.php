...
class InfoFlotBot
{
    ...
    {
        return $this->getAppsConfig()[$_REQUEST['auth']['application_token']];
    }

    protected function deleteApplicationConfig(): void
    {
    ...
    function writeToLog($var, string $logName, ?bool $append = false)
    {
        ...
        file_put_contents($dirPath . $logName . '.txt', $data, $append ? FILE_APPEND : 0);
    }

    protected function saveParams($params): void
    {
        $this->writeToLog(['$params' => $params], 'saveParams');
    ...
    protected function restCommand($method, array $params = [], array $auth = [])
    {
        $queryUrl = 'https://' . $auth['domain'] . '/rest/' . $method;
        $params = array_merge($params, ['auth' => $auth['access_token']]);

        $queryData = http_build_query($params);

        $this->writeToLog(['URL' => $queryUrl, 'PARAMS' => $params], 'restCommand', true);
        ...
        $this->writeToLog(['result' => $result], 'restCommand', true);
        return $result;
    }

    protected function getAnswer(string $command = '', ?int $userId = null): array
    {
        $this->writeToLog(['$command' => strtolower($command), '$userId' => $userId], 'getAnswer');
        ...
        switch (strtolower($command)) {
            ...
            case 'Ты заработал?':
                $arResult = [
                    'title' => 'Конечно',
                    'report' => 'Жду Апанасенко Павла для дальнейшей настройки',
                ];
                break;
            case 'Подобрать Круиз?':



                protected function onImBotJoinChat(): void
            {
                if (!$this->getApplicationConfig()) {
                    return;
                }
                ...
                $this->restCommand('imbot.message.add', $restParams, $_REQUEST["auth"]);
            }
                ...
                protected function onAppInstall(): void
            {
                ...
                $restParams = [
                    ...
                    'PROPERTIES' => [ // Личные данные чат-бота (обяз.)
                        ...
                        'WORK_POSITION' => 'Тестовый бот инфофлот',
                ...
                $result = $this->restCommand('imbot.register', $restParams, $_REQUEST["auth"]);
                ...
                protected function eventHandler(string $eventName)
            {
                $this->writeToLog(['$eventName' => $eventName], 'eventHandler');
            ...
            {
                $this->writeToLog(['$_REQUEST' => $_REQUEST], 'execute');

                if (!empty($_REQUEST['event'])) {
                    $this->eventHandler($_REQUEST['event']);
                }
            }
        }


        $configFilePath = InfoFlotBot::getConfigFilePath();
        if (!empty($configFilePath) && file_exists($configFilePath)) {
            include_once $configFilePath;
        }

        $appsConfig = !empty($appsConfig) ? $appsConfig : [];
        (new InfoFlotBot($appsConfig))->execute();
